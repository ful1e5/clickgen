...
class TestMain(unittest.TestCase):
    ...
    def test_get_config(self):
        ...
        mock_configs = clickgen.get_configs(directory=self.mock_config_dir_with_configs)
    ...
    def test_is_animated(self):
        ...
        self.assertTrue(clickgen.is_animated(config=assets.animated_mock_config_path))

        # testing config file without aimation
        self.assertFalse(clickgen.is_animated(config=assets.static_mock_config_path))

    def test_get_cur_name(self):
        # window extensions
        self.assert_cur_name(
            config=assets.animated_mock_config_path,
            type="win",
            expect_ext="mock_animated.ani",
        )
        self.assert_cur_name(
            config=assets.static_mock_config_path,
            type="win",
        ...
        self.assert_cur_name(
            config=assets.static_mock_config_path, type="x11", expect_ext="mock_static"
        )
    ...
    def test_archive(self):
        ...
        mock_out_file = os.path.join(self.mock_out_path, self.mock_name + ".tar")
