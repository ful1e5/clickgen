...
static int read_config_file(const char *config, struct flist **list) {
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    ...
    switch (sscanf(line, "%d %d %d %3999s %d", &size, &xhot, &yhot, pngfile,
                   &delay)) {
      case 4:
        delay = 50;
        break;
...
static void premultiply_data(png_structp png, png_row_infop row_info,
                             png_bytep data) {
  UNUSED(png);
  int i;

  for (i = 0; (unsigned)i < row_info->rowbytes; i += 4) {
...
static XcursorImage *load_image(struct flist *list, const char *prefix) {
  ...
  png_get_IHDR(png, info, &width, &height, &depth, &color, &interlace, NULL,
               NULL);

  /* TODO: More needs to be done here maybe */

  if (color == PNG_COLOR_TYPE_PALETTE && depth <= 8) png_set_expand(png);
...
static int write_cursors(int count, struct flist *list, const char *filename,
                         const char *prefix) {
...
static int check_image(char *image) {
  ...
  printf("%s: ", image);
  for (i = 0; i < XCURSOR_BITMAP_HASH_SIZE; i++) printf("%02x", hash[i]);
  printf("\n");
  return 0;
}
