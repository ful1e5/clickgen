...
import ctypes
from ctypes import CDLL, _CData, pointer
from os import path
...
LP_c_char: Type[pointer[_CData]] = ctypes.POINTER(ctypes.c_char)
LP_LP_c_char: Type[pointer[_CData]] = ctypes.POINTER(LP_c_char)
xcursorgen.main.argtypes = (ctypes.c_int, LP_LP_c_char)


def gen_argv_ctypes(argv: List[str]) -> pointer[_CData]:
...
def generate_x11_cursor(cfg_file_path: str, out_path: str, prefix: str) -> None:
    ...
    kwargs: pointer[_CData] = gen_argv_ctypes(argv)
